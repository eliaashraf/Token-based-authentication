...
const User = function(user){
  ...
  if( typeof user.created_at != 'undefined' ){
    this.created_at = user.updated_at;
  }
};
...
User.findBy = async (data, field, attach_additional_data = false) => {
  ...
  if(row.length){
    ...
    if(attached_additional_data){
       let addresses = [];

       try{
         address = await UserAddress.findByUserId(user.id);
       }catch(err){
         console.log("Error finding USER ADDRESS.", err);
       }
...
User.login = async (value) => {
  let row = await sql.query(`SELECT * FROM user WHERE = ? OR email = ?`, [value, value]);
  if(row.length){
    return row[0];
  }else{
    throw new NotFoundError("User does not exist");
